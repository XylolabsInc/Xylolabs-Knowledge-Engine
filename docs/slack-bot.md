...
3. **Gemini model** — `gemini-3.8-flash` is recommended; see [Gemini models](https://ai.google.dev/models) for alternatives
...
Setting `LLM_ENDPOINT` (see [README.md](../README.md#gemini-ai)) routes the bot through an OpenAI-compatible provider such as OpenRouter instead of native Gemini — use a provider-prefixed `GEMINI_MODEL` (e.g. `google/gemini-3.8-flash`) in that mode. Google Search grounding is silently disabled whenever `LLM_ENDPOINT` is set.
